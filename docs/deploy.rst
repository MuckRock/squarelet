...
.. envvar:: DJANGO_ALLOWED_HOSTS
    ...
    This sets the Django settings :setting:`ALLOWED_HOSTS`.  It specifies which domains you may use to access this site.  For staging, it is set to ``$HEROKU_APP_NAME.herokuapp.com`` (using the environment variable ``$HEROKU_APP_NAME``).  For production it will be set to ``accounts.muckrock.com``.
