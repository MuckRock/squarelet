...
class TestOrganization:
    ...
    def test_add_subscription_sells_one_unit_of_a_priced_plan(
        ...
        """A flat Price bills unit_amount x quantity.

        The views hand `add_subscription` no quantity, and it used to fill
        in `plan.minimum_users` - which the legacy tiered Plans priced as
        the base, and a flat Price prices as five of them.  Production's
        canonical Organization row has a minimum of 5.
        """
        user = user_factory()
        ...
        mock_sub_service.create.assert_called_once()
