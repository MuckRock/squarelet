...
class Organization(AvatarMixin, models.Model):
    ...
    def set_subscription(self, token, plan, max_users, user, payment_method=None):
        ...
        active_sub = self.subscriptions.filter(cancelled=False).first()
        from_plan = active_sub.plan if active_sub else None

        if not from_plan and plan:
            self.add_subscription(
                plan, max_users, user,
                token=token, payment_method=payment_method,
            )
        elif from_plan and not plan:
            self.remove_subscription(from_plan, user)
        elif from_plan and plan:
            if token:
                self.save_card(token, user)
            self.modify_subscription(from_plan, plan, max_users, user)

    def subscription_cancelled(self, subscription=None):
        """The subscription was cancelled due to payment failure"""
        target = subscription or self.subscriptions.first()

        # Create change log entry
        self.change_logs.create(
            reason=ChangeLogReason.failed,
            from_plan=target.plan if target else None,
            from_max_users=self.max_users,
            to_max_users=self.max_users,
        )

        # Cancel subscription in Stripe if it exists
        if target and target.subscription_id:
            try:
                stripe_sub = target.stripe_subscription
                if stripe_sub:
                    stripe_sub.delete()
                    logger.info(
