...
class TestAirtableVerificationUrl:
    ...
    def test_complete_data(self, mock_context, organization_factory):
        ...
        assert query_params["prefill_Email address on your account"] == ["john@example.com"]
        assert query_params["prefill_Organization or Project Name"] == ["Acme Corporation"]
        ...
        assert query_params["prefill_MuckRock Organization URL"] == ["/organizations/acme/"]
    ...
    def test_organization_without_get_absolute_url(self, mock_context, mock_organization):
...
class TestAirtableTemplateTagsInTemplate:
    ...
    def test_airtable_verification_url_in_template(self, user_factory, organization_factory):
        ...
        template = Template(
            "{% load airtable %}"
            "{% airtable_verification_url organization %}"
        )
    ...
    def test_integration_with_real_user_data(self, user_factory, organization_factory):
        ...
        assert result.startswith("https://airtable.com/app93Yt5cwdVWTnqn/pagogIhgB1jZTzq00/form")
        ...
        assert query_params["prefill_Organization or Project Name"] == [organization.name]
        
        # Check URLs (using real get_absolute_url methods)
        assert query_params["prefill_MuckRock Account URL"] == [user.get_absolute_url()]
        assert query_params["prefill_MuckRock Organization URL"] == [organization.get_absolute_url()]
